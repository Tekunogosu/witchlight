 ...
 *
 * Every marker, claim and plugin mark goes through here, so all three behave the
 * same way and a fourth kind joins the machinery rather than growing a copy of
 * it. Only one thing is `hovered` at a time: crossing from a marker to a claim
 * closes the first, so two boxes are never open at once.
 *
 * The setting is read at the moment of the hover rather than when the thing is
 * drawn, so switching it on reaches what is already on the map instead of only
 * the next set to arrive. Nothing happens while it is off, which is the point of
 * doing this here: a plugin cannot see that setting.
 *
 * A popup opened on purpose stays open. Only what a hover put up comes down on
 * its own, and a click is not a hover however it started.
 */
