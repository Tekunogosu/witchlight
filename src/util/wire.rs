//! Base64 decoding for byte payloads on the API channel.
//!
//! The alphabet is the plain one that .NET's `Convert.ToBase64String` writes,
//! because the other end of every wire here is a C# serializer.

/// Decodes base64 text into bytes, or returns why it could not.
...
mod tests {
    ...
    fn bytes_encoded_the_way_dotnet_would_decode_back() {
        // "AQIDBAUG" is base64 for bytes 1..=6, what .NET's encoder produces
        // for a six-byte value that needs no padding.
