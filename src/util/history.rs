//! A record of what changed at each generation, kept long enough for a viewer
//! to catch up.
//!
//! A browser asks what changed since generation G. It gets every entry newer
//! than G, or nothing at all when an entry it needed has already been dropped,
//! which the caller turns into a full repaint.
//!
//! The window is a length of time rather than a count of entries. Entries arrive
//! as fast as the server is busy, so a count that gives minutes of slack with
//! one player exploring gives seconds with forty. A hard cap sits underneath so
//! an unusually busy server cannot grow this without bound.
//!
//! The entry type is generic because the map's history names tiles and each
//! person's history names regions.

use std::collections::VecDeque;
use std::time::{Duration, Instant};

/// How long an entry is kept. Long enough that a tab left in the background
/// still picks up where it was.
pub const KEPT_FOR: Duration = Duration::from_secs(10 * 60);

/// The most entries kept regardless of age. At a thousand a minute, well past
/// what any server has produced, this is about a megabyte.
const MOST: usize = 50_000;

pub struct History<T> {
    entries: VecDeque<(u64, Instant, T)>,
    /// The newest generation that has been dropped. A reader from before it
    /// has missed something this can no longer name.
    dropped_up_to: u64,
}
...
impl<T> History<T> {
    /// Records what changed at `generation` and drops whatever has aged out.
    ...
    /// Returns everything newer than `since`, oldest first. Returns `None` when
    /// part of that range has already been dropped, meaning the caller must
    /// repaint everything.
