//! Owns the zoom levels above the finest, in memory and on disk.
//!
//! A level tile is a picture derived from the ground. See
//! [`crate::render::pyramid`] for the shape of the pyramid. The ground arrives a
//! few chunks at a time, so a tile over ground that is filling in would be
//! written on every build beat, a few hundred times an hour, for a picture whose
//! final form is the only one anybody keeps. This module holds the tile the
//! builder made and writes it once it has been quiet for a while, or once it has
//! waited long enough, whichever comes first. A walk that rebuilds the same tile
//! every two seconds then costs one write per quiet spell rather than one per
//! beat.
//!
//! Every reader of a level tile asks here, so what is in memory is what is
//! served and what the next level up is built from. The file holds the same
//! picture later. A tile lost with the process before it was written is not lost
//! ground, because the ground is in the database. It is built again on the next
//! start, which compares each region's last change against its level tiles'
//! files and marks anything behind. See `settle` in [`crate::server`].
//!
//! This is write-behind caching. Reads are answered from memory first and the
//! disk second. Writes land in memory and reach the disk on their own clock.
...
/// Ground filling in under a tile rebuilds it every beat, so a tile nobody has
/// rebuilt for this long is one the walk has moved past.
pub const QUIET_FOR: Duration = Duration::from_secs(30);

/// The longest a tile waits unwritten however often it is rebuilt, so a walk
/// that never stops still leaves a bounded backlog for the next start.
...
/// decoded. Past this the tile waiting longest is written early.
const MOST_HELD: usize = 64;

/// One tile the builder made that the disk has not seen yet.
struct Built {
    image: RgbImage,
    /// When the builder last made it. The quiet period counts from here, and a
    /// flush in flight compares against it to see whether it is still current.
    built: SystemTime,
    /// When it first went unwritten. The ceiling counts from here.
...
impl Levels {
    ...
    /// Takes a tile the builder has just made. It is served and built from
    /// immediately and written once it has been quiet. See
    /// [`flush`](Self::flush).
    ...
    /// Returns one tile as a picture, from memory or else from disk. Returns
    ...
    /// Returns one tile as PNG bytes for serving, encoded from memory or read
    /// from the file. A tile neither holds is reported as missing rather than
    ...
    /// Returns the best stored picture of a tile's ground from a level above it.
    ...
    /// Returns how tall the pyramid is, counting the tallest level on disk or in
    /// memory. A level that exists only in memory counts, so the builder is not
    /// told to make it again on the next beat.
    #[must_use]
    ...
    /// Returns how many tiles are waiting to be written, for the log.
    #[must_use]
    pub fn waiting(&self) -> usize {
        self.lock().len()
    }

    /// Writes and releases every tile that has been quiet for [`QUIET_FOR`] or
    /// unwritten for [`DIRTY_FOR`]. Returns how many were written.
    ...
    /// Writes these tiles and forgets them. A tile rebuilt while its picture was
    /// on the way to disk is kept and waits its own turn.
    ///
    /// The lock is not held across the write. A picture is copied out, written,
    /// and only then compared and released.
...
mod tests {
    ...
    fn memory_wins_over_disk_and_a_rebuild_during_a_write_is_kept() {
        ...
        // flush writes the picture it finds and keeps it, because it is the one
        // built last.
